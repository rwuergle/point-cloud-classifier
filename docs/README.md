...
* $S_\lambda = \frac{\lambda_3}{\lambda_3}$
...
## GeometricFeatureCalculator(point_cloud_path: str, slope: float | None = None)
...
### compute_relative_z(feature_names: list[str] = ["z_norm"]) -> None
...
### compute_3d_features(features: dict[str, list[float]]) -> None:
...
### compute_2d_features(features: dict[str, list[float]]) -> None:
